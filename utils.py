...
class EpisodicDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, 'r') as root:
            # Check if simulation data
            is_sim = root.attrs.get('sim', False)
            compressed = root.attrs.get('compress', False)
            
            # Load action data - (N, 10) format: [base_vel(3) + d1_pos(7)]
            action = root['/action'][()]
            original_action_shape = action.shape
            episode_len = original_action_shape[0]
            
            # Get D1 arm observation at start_ts (7 dimensions)
            qpos = root['/observations/arm/joint_positions'][start_ts]  # 7D
            
            # Get camera images
            image_dict = dict()
            for cam_name in self.camera_names:
                cam_path = f'/observations/images/{cam_name}'
                if cam_path not in root:
                    raise KeyError(f"Camera {cam_name} not found in {dataset_path}")
                image_dict[cam_name] = root[cam_path][start_ts]
...
def get_norm_stats(dataset_path_list):
    ...
    for dataset_path in dataset_path_list:
        with h5py.File(dataset_path, 'r') as root:
            # Load D1 joint positions (7D)
            qpos = root['/observations/arm/joint_positions'][()]
...
def load_data(dataset_dir_l, name_filter, camera_names, batch_size_train, batch_size_val, chunk_size, 
    ...
    train_episode_ids_l = [train_episode_ids_0] + [np.arange(num_episodes) + num_episodes_cumsum[idx] 
    ...
    print(f'\n\nData from: {dataset_dir_l}\n- Train on {[len(x) for x in train_episode_ids_l]} episodes\n'
          f'- Test on {[len(x) for x in val_episode_ids_l]} episodes\n\n')
    ...
    train_num_workers = 8 if train_dataset.augment_images else 8
    val_num_workers = 8 if train_dataset.augment_images else 4
    
    print(f'Augment images: {train_dataset.augment_images}, train_num_workers: {train_num_workers}, '
